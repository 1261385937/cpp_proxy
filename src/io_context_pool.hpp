...
private:
    ...
    std::vector<std::unique_ptr<asio::io_context>> io_context_vec_;
    std::vector<std::unique_ptr<asio::io_context::work>> work_vec_;  // keep io_context.run() alive
public:
    explicit io_context_pool(uint32_t pool_size) : io_context_index_(0) {
        for (uint32_t i = 0; i < pool_size; ++i) {
            auto io_context_ptr = std::make_unique<asio::io_context>(1);
            auto work_ptr = std::make_unique<asio::io_context::work>(*io_context_ptr);
    ...
    void start() {
        ...
        for (size_t i = 0; i < size; ++i) {
            auto thread_ptr = std::make_unique<std::thread>([this, i]() {
                try {
                    io_context_vec_[i]->run();
                } catch (std::exception& e) {
                    LOG_ERROR("io_service thread exit with {}", e.what());
                } catch (...) { LOG_ERROR("io_service thread exit with unknown exception"); }
            });
